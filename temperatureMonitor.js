  ...
  Object.keys(allTemps).forEach(async function (key) {
    try {
      ...
      user.thermostats.forEach((thermostat) => {
        ...
        console.log(
          `Current target temp ${user.getCurrentTargetTemp(
            thermostat
          )} Reported value: ${user.getCurrentReportedValue(
            thermostat
          )} Desired max temp ${row.maxTemp} Temperature to set if above max ${
            row.tempToSet
          }`
        );
      });
    } catch (e) {
      console.log('An issue occured ', e);
    }
  });
};
